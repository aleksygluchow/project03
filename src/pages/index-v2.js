...
export default function Home() {
  return (
    <>
      ...
      <Layout>
        {/* 🔥 Hide ANY layout-level nav/header that isn't inside this page */}
        <style jsx global>{`
          :where(header, nav, [role="navigation"]):not(#retro-root :where(header, nav, [role="navigation"])) {
              ...
              <tbody>
                <tr>
                  ...
                  <td className="content">
                    ...
                    <div className="intro">
                      <p style={{ fontFamily: "'UnifrakturCook', cursive", fontSize: '1.25rem', marginTop: 0 }}>
                        Jetzt in Polen{' '}
                        <a href="https://de.wikipedia.org/wiki/Polen" target="_blank" rel="noopener noreferrer">
                          <img src="https://flagcdn.com/w40/pl.png" alt="Polnische Flagge" width="20"
        ...
        <style jsx>{`
          ...
          .outer { width: 100%; max-width: 980px; margin: 0 auto; border: 4px ridge #39f; background: rgba(0,0,0,0.65); }

          .sidebar { width: 240px; background: repeating-linear-gradient(45deg,#0a0,#0a0 8px,#070 8px,#070 16px);
                     border-right: 3px groove #39f; vertical-align: top; }
          ...
          .badge { display: block; padding: 6px 8px; border: 2px inset #39f; background: linear-gradient(#003,#001);
                   font-size: 12px; text-align: center; }

          .content { background: #001; }
          .header { padding: 14px 16px; background: linear-gradient(#004,#002); border-bottom: 3px groove #39f; }
          .site-title { margin: 0; font-size: 32px; font-family: Impact, "Arial Black", sans-serif; color: #0ff; }
          .subtitle { font-size: 12px; color: #ff0; margin-top: 4px; }

          .blink { animation: blink 0.9s steps(2, start) infinite; background: #f0f; padding: 1px 4px; color: #000; margin-right: 6px; }
          @keyframes blink { to { visibility: hidden; } }

          .marquee { overflow: hidden; border-bottom: 3px ridge #39f; background: #111; }
          .marquee-inner { display: inline-block; padding: 8px 0; white-space: nowrap; animation: slide 12s linear infinite; font-weight: bold; }
          @keyframes slide { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } }

          .intro { padding: 16px; }
          .copy { font-size: 14px; line-height: 1.45; }
          .buttons { margin: 14px 0; display: flex; gap: 10px; flex-wrap: wrap; }
          .btn { display: inline-block; padding: 10px 14px; border: 3px outset #39f; background: linear-gradient(#0af, #07c);
                 color: #fff; font-weight: bold; box-shadow: 0 0 10px #0ff; }
          .btn:active { border: 3px inset #39f; }

          .counter { margin-top: 8px; font-family: "Courier New", monospace; background: #000; display: inline-block;
                     padding: 4px 8px; border: 2px inset #39f; }
          .footer { margin-top: 10px; padding: 12px 16px; background: #111; border-top: 3px groove #39f; text-align: center; font-size: 12px; color: #ddd; }
